...
흐름:
  ...
  2) 크로미움 계열 브라우저를 전용 프로필 + 디버그 포트(CDP)로 띄워 attach → 로그인된 chatgpt.com 세션 재사용
  3) 패킹본을 '파일 첨부' + 짧은 프롬프트로 투입 (모델/추론단계 검증)
  4) 턴 단위로 응답 완료를 판정(stop-button 사라짐 + copy 버튼 등장 + 텍스트 안정) → 회수
  5) 응답을 .md로 원자적 저장

Codex 포트 (insane-review-codex):
  - 이 스크립트는 순수 Python이며 본진(Codex CLI)판 bin/pack_and_ask.py와 1:1로 동일한 엔진이다.
  - 경로 기준: $PLUGIN_ROOT 환경변수 또는 이 스크립트의 부모 디렉토리(scripts/). CODEX_PLUGIN_ROOT 의존 없음.
  - 출력: 실행한 현재 프로젝트의 .insane-review/ (env INSANE_REVIEW_OUT 또는 --out-dir로 오버라이드).

진화 이력:
  v2 (2026-06-20): 턴-스코프 판정, 모델 검증, fail-closed CDP/로그인, force-answer 재시도,
    UUID/PID 파일명, repomix 버전 핀+timeout, 권한/시크릿, env 설정화.
  v0.3.x: 폴더명 ChatGPT '프로젝트' 그룹핑(캐시→탐색→생성, 실패 시 일반채팅 폴백) + 하드닝.
  v0.4.x: 크로스플랫폼(mac/win/linux) 브라우저 스캔/실행 + 전용(격리) 프로필 + insert_text 입력 +
    싱글톤 교착 자가복구. (Chrome 136+는 전용 프로필 없이는 CDP가 안 열린다.)
  v0.5.x: GPT-5.5 Pro 셀프리뷰 반영 — hermetic repomix config, 첨부→인라인 폴백, 동명 폴더 분리,
    CDP 다이얼로그 레이스 핸들링.
"""
...
def check_env(do_install: bool = False) -> int:
    ...
    마지막에 'STATUS ...' 라인을 출력해 커맨드(question prompt 온보딩)가 분기에 파싱한다."""
    import importlib.util
    ...
    # 설치된 크로미움 목록 — 커맨드가 브라우저 선택 question prompt을 구성하는 데 사용
    bs = detect_browsers()
...
def read_menu_state(page) -> dict:
    ...
    try:
        ...
        for it in page.query_selector_all('[role="menuitem"], [role="menuitemradio"], [role="option"]'):
            ...
            if t and re.search(r"GPT|gpt|o\d|Codex|Gemini", t):
