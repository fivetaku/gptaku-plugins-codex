...
`scripts/pack_and_ask.py`의 `--council` 모드가 그 간극을 메운다 — **프롬프트를 위치인자로 받고, 응답만 stdout으로** 내보내(진행 로그는 stderr) council worker가 `output.txt`로 그대로 캡처한다.
...
- Comet/Chrome이 디버그포트(9222)로 실행 + chatgpt.com 로그인 + 모델 Pro.
