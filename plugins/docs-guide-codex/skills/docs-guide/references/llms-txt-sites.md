# Known llms.txt Sites

Last verified: 2026-02-20

## Verified Sites — 68개

### Frontend (10)

| Library/Service | llms.txt URL |
|----------------|-------------|
| React | https://react.dev/llms.txt |
| Next.js | https://nextjs.org/llms.txt |
| Vue | https://vuejs.org/llms.txt |
| Svelte | https://svelte.dev/llms.txt |
| Angular | https://angular.dev/llms.txt |
| Astro | https://docs.astro.build/llms.txt |
| Nuxt | https://nuxt.com/llms.txt |
| Chakra UI | https://chakra-ui.com/llms.txt |
| Mantine | https://mantine.dev/llms.txt |
| Nx (monorepo) | https://nx.dev/llms.txt |

### Backend (2)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Django | https://docs.djangoproject.com/llms.txt |
| Hono | https://hono.dev/llms.txt |

### Database / ORM (7)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Prisma | https://www.prisma.io/docs/llms.txt |
| Supabase | https://supabase.com/llms.txt |
| Drizzle ORM | https://orm.drizzle.team/llms.txt |
| TypeORM | https://typeorm.io/llms.txt |
| MongoDB | https://www.mongodb.com/llms.txt |
| Redis | https://redis.io/llms.txt |
| Qdrant (Vector DB) | https://qdrant.tech/llms.txt |

### Vector / Graph DB (5)

| Library/Service | llms.txt URL |
|----------------|-------------|
| ChromaDB | https://docs.trychroma.com/llms.txt |
| Pinecone | https://docs.pinecone.io/llms.txt |
| Weaviate | https://docs.weaviate.io/llms.txt |
| Milvus | https://milvus.io/llms.txt |
| Neo4j (⚠️ 마케팅 인덱스) | https://neo4j.com/llms.txt |

> **Neo4j 주의**: `neo4j.com/llms.txt`는 마케팅 사이트 인덱스. Cypher 등 기술 문서는 `neo4j.com/docs/cypher-manual/current/`에서 직접 WebFetch.

### Serverless DB / Platform DB (2)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Neon (Serverless Postgres) | https://neon.tech/llms.txt |
| Turso (SQLite-compatible) | https://turso.tech/llms.txt |

### Messaging (1)

| Library/Service | llms.txt URL |
|----------------|-------------|
| NATS | https://docs.nats.io/llms.txt |

### Payment / Auth (3)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Stripe | https://docs.stripe.com/llms.txt |
| Clerk | https://clerk.com/llms.txt |
| Auth0 | https://auth0.com/ai/docs/llms.txt |

### Platform / Cloud (6)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Vercel | https://vercel.com/llms.txt |
| Docker | https://docs.docker.com/llms.txt |
| GitHub Docs | https://docs.github.com/llms.txt |
| AWS Docs | https://docs.aws.amazon.com/llms.txt |
| Cloudflare | https://developers.cloudflare.com/llms.txt |
| Netlify | https://docs.netlify.com/llms.txt |

### Tooling / Build (7)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Vite | https://vite.dev/llms.txt |
| Vitest | https://vitest.dev/llms.txt |
| Turborepo | https://turborepo.dev/llms.txt |
| Bun | https://bun.sh/llms.txt |
| Deno | https://docs.deno.com/llms.txt |
| Prettier | https://prettier.io/llms.txt |
| Oxc | https://oxc.rs/llms.txt |

### Validation / API (2)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Zod | https://zod.dev/llms.txt |
| tRPC | https://trpc.io/llms.txt |

### Search / Monitoring (3)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Meilisearch | https://www.meilisearch.com/llms.txt |
| Algolia | https://www.algolia.com/doc/llms.txt |
| Datadog | https://docs.datadoghq.com/llms.txt |

### CMS / Headless (3)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Strapi | https://docs.strapi.io/llms.txt |
| Ghost | https://docs.ghost.org/llms.txt |
| Sanity | https://www.sanity.io/docs/llms.txt |

### AI/ML (2)

| Library/Service | llms.txt URL |
|----------------|-------------|
| LangChain | https://python.langchain.com/llms.txt |
| CrewAI | https://docs.crewai.com/llms.txt |

### LLM Providers (4)

| Library/Service | llms.txt URL |
|----------------|-------------|
| OpenAI | https://platform.openai.com/docs/llms.txt |
| Mistral | https://docs.mistral.ai/llms.txt |
| Cohere | https://docs.cohere.com/llms.txt |
| Perplexity | https://docs.perplexity.ai/llms.txt |

### AI Dev Tools / Infra (7)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Windsurf (Codeium) | https://docs.windsurf.com/llms.txt |
| Tabnine | https://docs.tabnine.com/llms.txt |
| LiteLLM | https://docs.litellm.ai/llms.txt |
| Together AI | https://docs.together.ai/llms.txt |
| Fireworks AI | https://docs.fireworks.ai/llms.txt |
| Replicate | https://replicate.com/docs/llms.txt |
| Databricks | https://docs.databricks.com/llms.txt |

### AI Observability (1)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Weights & Biases | https://docs.wandb.ai/llms.txt |

### Language (1)

| Library/Service | llms.txt URL |
|----------------|-------------|
| Kotlin | https://kotlinlang.org/llms.txt |

### Mobile (2)

| Library/Service | llms.txt URL |
|----------------|-------------|
| React Native | https://reactnative.dev/llms.txt |
| Expo | https://docs.expo.dev/llms.txt |

> Note: Anthropic product docs are excluded from this generic index and should be handled by a dedicated product-docs workflow.

## URL Patterns to Try

When a library is not in the list above, try these patterns in order:

1. `{official-site}/llms.txt`
2. `{official-site}/docs/llms.txt`
3. `{official-site}/llms-full.txt`
4. `{official-site}/docs/llms-full.txt`

## How to Verify

Fetch the URL with WebFetch. A valid llms.txt:
- Returns HTTP 200
- Contains structured text with links to documentation pages
- May include section headers, page titles, and URLs
- Note: some sites redirect (e.g., vitejs.dev → vite.dev, neon.tech → neon.com). Follow redirects.
- Note: some sites serve llms.txt at non-root paths (e.g., Algolia at `/doc/llms.txt`, not `/llms.txt`)
