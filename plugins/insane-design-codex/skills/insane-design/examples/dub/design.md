...
token_prefix: --bg-*, --border-*, --content-*
...
순백 캔버스 위 검정 CTA + violet accent을(를) 축으로 하는 디자인 시스템. 링크 어트리뷰션 분석 플랫폼 — 순백 배경 + 얇은 그리드 + 검정 CTA + 보라 한 점.

Dub의 마케팅 홈은 refined minimalism 성격을 유지한다. 브랜드 컬러 `#855AFC`는 CTA, 링크, focus ring 등 의미가 필요한 지점에만 등장하며, 넓은 면적은 light 캔버스(`#FFFFFF` 계열)가 담당한다. 이 구조는 사용자가 "콘텐츠에 집중하게 하고, 색이 개입하는 순간은 결정이 필요한 순간"이라는 일관된 규율로 설계되었다.

색상 전략은 한 점 accent + neutral surface의 고전 SaaS 문법을 따른다. 체감상 가장 넓은 면적은 배경과 텍스트이고, `#855AFC`는 전체 픽셀의 5% 미만이다. 이런 절제가 "Dub다움"을 만든다.
...
- near-white monochrome
- violet brand accent
- Geist stack
- link attribution analytics
- grid texture BG
...
> **Signature Element**: `minimal_extreme`
> **Code Complexity**: low
...
**절대 하지 말아야 할 것 하나**: #855AFC를 본문 텍스트 색이나 긴 문단 배경으로 쓰지 말 것. Dub의 정체성은 brand accent를 **한 점**에만 올리는 절제다. 넓은 면적으로 가져가면 즉시 다른 제품이 된다.
...
| Token prefix | `--bg-*, --border-*, --content-*` |
...
- **Design system**: 자체 `--bg-*/--border-*/--content-*` RGB triplet 토큰
- **CSS architecture**: Tailwind utility + CSS vars (rgb triplet 방식)
- **Class naming**: Next module + Tailwind
- **Default theme**: light (#FFFFFF + #FAFAFA muted)
- **Font loading**: self-host Geist + Geist Mono + Geist Dash
- **Canonical anchor**: #855AFC (violet) — feature tab 'Affiliate Programs' accent
- **Hero**: 중앙 정렬 H1 + 검정 CTA + 보조 light CTA + grid texture BG
...
| Token | Hex | Role |
|---|---|---|
| brand.primary | `#855AFC` | CTA, link, focus ring |
...
> Dub 회색 램프는 Tailwind neutral 계열과 유사 — 채도 0% 기준.
...
```js
// tailwind.config.js — Dub
module.exports = {
  theme: {
    extend: {
      colors: {
        brand: { DEFAULT: '#855AFC' },
      },
      fontFamily: {
...
**Hero Section**
> Dub 스타일 히어로. 배경 `#FFFFFF`, H1 `Geist` 56px weight 700 tracking -0.03em color `#171717`. primary CTA `#855AFC` radius 8px padding 0 20px height 44px, secondary outline border `#E5E5E5`.

**Card**
> Dub 스타일 카드. bg `#FFFFFF` border 1px solid `#E5E5E5` radius 12px padding 24px. hover border `#855AFC40` shadow `0 4px 6px rgba(0,0,0,0.05)`.
...
### ✅ DO
- 브랜드 색 `#855AFC`는 CTA·링크·focus ring에만 사용
...
- body에 `font-weight: 300` 사용 금지 — Dub은 `400`이 기본
- 브랜드 색 `#855AFC`를 긴 문단 배경에 쓰지 말 것
