...
Included skills (Codex is skill-first — no `command-routes/` directory):
...
Codex adaptations vs the Codex CLI plugin:
- `question prompt` card UI does not exist; every menu/selection uses the numbered-options chat pattern from `shared/questioning-policy.md` §A.
