...
AI 활용자를 위한 멘토 에이전트. Codex 대화 기록을 분석하고 회고하여, **더 나은 AI 협업자**로 레벨업하세요.
...
5. **과잉코칭 가드**: 사용자가 이미 스스로 진단했거나 직접 답을 원하면, 더 캐묻지 말고 바로 제공한다
...
├── config/                   ← 설정 (온보딩이 생성)
│   ├── project_names.json    ← 워크스페이스 이름 매핑
...
모든 기능은 바선생 Codex 스킬로 실행합니다.

| 기능 | 설명 |
|------|------|
| "바선생 시작" (vibe-sunsang-onboard) | **처음 1회** — 워크스페이스 연결, 유형 분류, 초기 설정 |
| "변환해줘" (vibe-sunsang-retro) | 새 대화 변환 (매주 실행 권장) |
| "멘토링해줘" (vibe-sunsang-mentor) | AI 활용 능력 코칭 (유형별 맞춤, 4가지 모드) |
| "성장 리포트 만들어줘" (vibe-sunsang-growth) | 성장 분석 리포트 (유형별 맞춤, 종단 비교 포함) |

**추천 루틴:**
1. 매주 금요일, 이 폴더에서 `codex` 실행
2. "변환해줘"로 이번 주 대화 변환
3. "멘토링해줘"로 이번 주 리뷰
4. 행동 계획 실천
...
> 이 워크스페이스의 사용자는 **AI 활용 교육 강사**일 수 있다. 본인의 성장 추적뿐 아니라, 수강생을 가이드하기 위한 이해도 확보가 목적이다.
...
```
1단계: 유형 확인 → Builder/Explorer/Designer/Operator, 유형에 맞는 기준으로 해석하는지
2단계: 레벨 판정 근거 → 왜 이 레벨인지 구체적 증거, 다음 레벨로 가려면 뭐가 필요한지
3단계: 안티패턴 우선순위 → 여러 개면 가장 임팩트 큰 것 1개만 먼저 ("이번 주는 이것만")
4단계: 성장 인정 → 이전 대비 나아진 점을 먼저, 수치로 ("지난주 42% → 이번주 51%")
```
