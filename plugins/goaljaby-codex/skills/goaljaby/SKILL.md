---
name: goaljaby
description: PRD 폴더를 받아 한국어 검토 문서 5종(VALIDATION/RECOVERY/PLAN/PROGRESS 대응 + PLANS.md ExecPlan)을 생성하고, 검토 요약을 대화창에 띄운 뒤 사용자 승인을 받으면 복사해서 실행할 수 있는 Codex `/goal` 명령을 건넨다. 트리거 — "/goaljaby", "골잡이", "골 셋업 만들어줘", "PRD에서 골 만들어줘", "PRD 골 변환", "VALIDATION RECOVERY 만들어줘", "골 실행 준비", "set up goal from PRD", "goaljaby", "prep goal docs", "make goal scaffolding". PRD 폴더가 있고 `/goal` 기반 장시간 세션으로 넘어가고 싶을 때 사용한다.
---

# 골잡이 (goaljaby) — Codex 판

> PRD를 Codex `/goal` 운영 계약으로 변환하는 브릿지 스킬. 한국어 검토 문서 5종 + `PLANS.md` ExecPlan을 생성하고, 검토 요약을 대화창에 띄운 뒤 승인을 받으면 **복사해서 바로 실행할 수 있는 `/goal` 명령**을 건넨다.
...
├── goal-command.md    — `/goal` 실행 본문 (한국어, 4,000자 강제, ./PLANS.md 포인터)
...
- 5종 검토 문서 + PLANS.md는 모두 한국어 헤딩 + 한국어 본문. 영어 헤딩(Required Checks / Not Done If / Core Rule 등) 잔존 시 Step 7 검증에서 실패 → 결과물 폐기.
...
`references/templates.md`의 한국어 템플릿(VALIDATION/RECOVERY/PLAN/PROGRESS/goal-command + PLANS.md ExecPlan)을 PRD 내용에 맞춰 슬롯 치환. 작업 유형에 따라 강조 항목이 달라진다 — 매핑 표는 `references/task-type-templates.md` 참조.
...
7. **영어 헤딩 잔존 검사**: `grep -nE "^## (Required Checks|Targeted Checks|Manual Verification|Acceptance Criteria Mapping|Not Done If|Core Rule|Failure Loop|Retry Limit|Scope Control|Reorientation Rule|Revert Rule|Goal|Source Documents|Final Completion Criteria|Current Goal|Current Milestone|Completed|Last Validation|Failed Attempts|Current Best State|Next Step|Risks|Handoff Notes|Visual Verification)\b"` — 0건이어야 함 (단 PLANS.md의 `## Progress`/`## Validation`/`## Decision-Log`는 ExecPlan 표준 섹션명이므로 6번에서 의도적으로 허용한다). 1건 이상이면 한국어 템플릿 치환 실패 → 폐기.

하나라도 실패 → `compact-strategy.md` §구조적 오버플로우 보고로 분기. **6종 파일을 저장하지 않는다.**

### Step 8: 한국어 검토 요약 표시 + PROGRESS.md / PLANS.md prepend
**타입**: prompt + Bash

별도 파일을 만들지 않는다. 한국어 검토 요약을 **대화창에 직접 출력**하고, 핸드오프 용도로 **PROGRESS.md 상단 + PLANS.md의 Progress 섹션 상단에 4줄 요약을 prepend**한다.

**대화창 출력 형식** (5섹션 + 시작 직전 확인):
...
- **`references/templates.md`** — 6개 파일 한국어 템플릿(5종 + PLANS.md ExecPlan) + F-1~F-6 한국어 본문
- **`references/task-type-classifier.md`** — 한/영 키워드 기반 작업 유형 추정 규칙
- **`references/task-type-templates.md`** — 작업 유형 6종 × 파일 강조 항목 (한국어 additions)
...
- **검증 (Step 7)**: Bash `grep -P` + `python3 -c "print(len(...))"`로 PROTECTED_CLAUSES 정규식·문자수·PLANS.md 섹션·영어 헤딩 잔존을 결정론적으로 매칭
- **검토 요약 (Step 8)**: 대화창에 한국어 요약 출력 + PROGRESS.md/PLANS.md 상단에 4줄 요약 prepend
