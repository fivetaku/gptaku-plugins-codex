...
## Codex CLI 판과의 차이 (왜 Codex 판이 따로 있나)

본진 goaljaby는 "Codex CLI 전용"이다. 마지막에 어시스턴트가 응답의 *마지막 줄*로 `/goal`을 출력하면 Codex CLI가 그 줄을 다음 턴 입력으로 처리해 골 루프를 자동 시작한다. **Codex에는 이 메커니즘이 없다** — 스킬이 슬래시 명령을 마지막 줄로 "자동 발사"할 수 없다.
...
> **언어 잠금 (먼저)**: 워크플로우 시작 전 `shared/language-policy.md §1`로 `output_lang`을 감지·고정한다. 이후 모든 산출물(6종 문서·헤딩·대화 요약·Step 10 핸드오프 안내)은 `output_lang`을 따른다. 식별자/슬롯/PROTECTED_CLAUSES 앵커/PLANS.md ExecPlan 표준 섹션명은 제외(§2).
...
**중요**: Codex 스킬은 마지막 줄로 슬래시 명령을 자동 발사할 수 없다(그건 Codex CLI 메커니즘이다). 따라서 이 판은 **복사-실행 핸드오프**로 끝낸다 — 절대 "다음 턴에 자동 시작된다"고 약속하지 말 것.
