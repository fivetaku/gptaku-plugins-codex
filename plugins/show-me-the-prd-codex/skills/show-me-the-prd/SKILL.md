...
Codex CLI에는 Codex CLI의 `question prompt`(객관식 카드 UI)에 해당하는 도구가 **없다.** 결정이 필요한 질문은 모두 **채팅에 번호형 선택지 블록을 출력하고 사용자의 다음 자유 텍스트 답변을 읽는** §A 패턴으로 한다.
...
- 구조화된 결정(데이터 모델 / Phase / 기술 스택 비교)에는 선택지 **위에** 짧은 예시 프리뷰(ASCII 관계도·테이블·트리)를 먼저 보여준다 — Codex의 `preview` 필드 대체.
